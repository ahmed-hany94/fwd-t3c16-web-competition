import { insertQuery, selectAllQuery, truncateQuery } from '../db';
...
describe('Test database queries', () => {
  ...
  it("Shouldn't create user with the same email", async () => {
    ...
    expect(operationMessage).toBe(
      `Insert Failed - Unique Violation: duplicate key value violates unique constraint "users_email_key"`
    );
  });
  ...
  it('Should select all users', async () => {
    ...
    const operationResult = await selectAllQuery(query);
  ...
  it('Should create a movie', async () => {
    // Start with a clean slate
    await truncateQuery('TRUNCATE TABLE movies CASCADE;');
  ...
  it("Shouldn't create a movie with the same name", async () => {
    ...
    const operationMessage = await insertQuery(query, params);
    expect(operationMessage).toBe(
      `Insert Failed - Unique Violation: duplicate key value violates unique constraint "movies_name_key"`
    );
  });
  ...
  it('Should create a watch_list', async () => {
    // Start with a clean slate
    await truncateQuery('TRUNCATE TABLE watch_list;');
